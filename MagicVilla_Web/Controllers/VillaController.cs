...
namespace MagicVilla_Web.Controllers
{
    public class VillaController : Controller
    {
        ...
        public async Task<IActionResult> CreateVilla(VillaCreateDTO  model)
        {
            ...
            {
                var responce = await _VillaServicec.CreateAsync<APIResponse>(model);
                if (responce != null && responce.IsSuccess)
                {
        ...
        {
            var responce = await _VillaServicec.GetAsync<APIResponse>(villaId);
            if (responce != null && responce.IsSuccess)
            {
        ...
        public async Task<IActionResult> UpdateVilla(VillaUpdateDTO model)
        {
            if (ModelState.IsValid)
            {
                ...
                var responce = await _VillaServicec.UpdateAsync<APIResponse>(model);
                if (responce != null && responce.IsSuccess)
                {
                    return RedirectToAction(nameof(IndexVilla));
                }
        ...
        public async Task<IActionResult> DeleteVilla(VillaUpdateDTO model)
        {
                ...
                var responce = await _VillaServicec.DeleteAsync<APIResponse>(model.Id);
                if (responce != null && responce.IsSuccess)
                {
